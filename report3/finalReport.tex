...
For the next point, it is necessary to consider the construction of $A$ in a few words. One axis is indexed by the number of pixels in the video plus one vertex for the sink and the other by the edges in the video's graph. Most entries of $A$ are zeros. In each column are one $1$ and one $-1$ indicating source and target of the edge. So $A$ is the incidence matrix of the graph and its rank is therefore the number of rows minus 1.\\
