...
\section{Tracking as Min-Cost-Flow Problem}
In our first approach we want to use a min-cost-flow. 
...
Since our first purpose was the application of this process to a minimum-cost flow problem (MCFP) for computing a shortest path through a graph associated to a video, we could restrict ourselves to an linear optimization problem of the following form:

\[\min_f c^T f\] 
\[ \text{subject to} ~~Af-b=0 ~\text{and}~ f\geq 0,\]

where $f$ is the flow through the video and $c$ is the cost vector related the graph's edges. Together with suitable $A$, $b$ and the inequality $f\geq 0$ this encode the MCFP. 
