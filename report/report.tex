...
In our case the top left entry of $M$ is the zero matrix in contrast to the positive definite matrix $Q$ in the OptNet paper, why $M$ is not invertible. So we can compute a solution (if it exists), which is of course not unique. With this solution for $\frac{\partial f}{\partial c}$ we can compute 
